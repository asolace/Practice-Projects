...
def panoramic_pairs(landmarks)
  panView = []
  landmarks.each_with_index do |places, idx|
    if idx != landmarks.length - 1
      panView << [places, landmarks[idx + 1]]
    else
      panView << [landmarks[-1], landmarks[0]]
    end
  end
  return panView
end
...
def two_degrees_away(facebook, name)
  ...
  facebook[name].each do |friend|
    facebook[friend].each do |friendFriend|
      if ((!facebook[name].include?(friendFriend)) && (friendFriend != name))
        fof << friendFriend
      end
    end
  end
...
def remove_letter_a(words)
  return words.map do |word|
    word.chars.reject {|char| char == 'a'}.join('')
  end
end
