...
def repeatCounter(word)
  counter = 0
  seen = []
  counted = []
  word.each_char do |letter|
    if seen.include?(letter) && !counted.include?(letter)
      counter += 1
      counted << letter
    else
      seen << letter
...
def word_with_most_repeats(sentence)
  ...
  most_repeat_ammount = 0
  most_repeat_word = nil
  words.each do |word|
    if repeatCounter(word) > most_repeat_ammount
      most_repeat_ammount = repeatCounter(word)
      most_repeat_word = word
    end
  end
  most_repeat_word
end
...
def isogram_matcher(isogram1, isogram2)
  ...
  isogram1.chars.each_with_index do |char1, i|
    isogram2.chars.each_with_index do |char2, j|
      samePosition += 1 if char1 == char2 && i == j
      notSame += 1 if char1 == char2 && i != j
    end
  end
  return [samePosition, notSame]
end
...
def cupcake_solver(cupcake_counts, number_of_students_in_class)
  return cupcake_counts.map { |cupcake| cupcake / number_of_students_in_class }.
  reduce(&:+)
end
